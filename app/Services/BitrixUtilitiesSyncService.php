...
class BitrixUtilitiesSyncService
{
    ...
    {
        $response = $this->call('lists.field.get', [
            'IBLOCK_TYPE_ID' => self::IBLOCK_TYPE_ID,
            'IBLOCK_ID' => $iblockId,
        ]);
    ...
    private function expandUtilitiesEnumMap(array $fieldMap, array $items): array
    {
        ...
        $observedIds = [];
        foreach ($items as $item) {
            $raw = $this->toNullableString($this->propertyValue($item, $fieldMap, 'UTILITIES'));
    ...
    /**
     * Call Bitrix REST method via inbound webhook (lists scope).
     *
    ...
    {
        $webhook = rtrim((string) config('services.bitrix.webhook'), '/').'/';
        if ($webhook === '/') {
    ...
    private function normalizeItem(
        ...
        $utilityTypeRaw = $this->toNullableString($this->propertyValue($item, $fieldMap, 'UTILITIES'));
        ...
        return [
            ...
            'bitrix_id' => $bitrixId,
            'name' => $this->toNullableString($item['NAME'] ?? null),
            'utility_type_id' => $utilityTypeId,
            'utility_type' => $utilityType,
            'provider_company' => $this->toNullableString($this->propertyValue($item, $fieldMap, 'PROVIDER_COMPANY')),
            'account_number' => $this->toNullableString($this->propertyValue($item, $fieldMap, 'ACCOUNT_NUMBER')),
            'login' => $this->toNullableString($this->propertyValue($item, $fieldMap, 'LOGIN_')),
            'password' => $this->toNullableString($this->propertyValue($item, $fieldMap, 'PASSWORD')),
            'email_for_registration' => $this->toNullableString($this->propertyValue($item, $fieldMap, 'EMAIL_FOR_REGISTRATION')),
            'name_used_for_registration' => $this->toNullableString($this->propertyValue($item, $fieldMap, 'NAME_USED_FOR_REGISTRATION')),
            ...
            'autopayment_date' => $this->toNullableDateTime($this->propertyValue($item, $fieldMap, 'AUTOPAYMENT_DATE')),
            'apartment_text' => $this->toNullableString($this->propertyValue($item, $fieldMap, 'APARTMENT_TEXT')),
            'is_deleted' => false,
            'bitrix_created_at' => $this->toNullableDateTime($item['DATE_CREATE'] ?? null),
            'bitrix_updated_at' => $this->toNullableDateTime($item['TIMESTAMP_X'] ?? null),
            'last_synced_at' => $now,
            'updated_at' => $now,
            'created_at' => $now,
        ];
    ...
    private function toNullableString(mixed $value): ?string
    {
        ...
        if (is_array($value)) {
            $value = $this->unwrapPropertyValue($value);
