...
class BitrixWebhookController extends Controller
{
    ...
    public function __invoke(Request $request, BitrixEntitySyncService $syncService): JsonResponse
    {
        ...
        [$tokenOk, $tokenChannel] = $this->tokenVerifier->verify($request, BitrixOutgoingWebhookContext::Crm);
        if (! $tokenOk) {
        ...
        if ($this->isContactDeleteEvent($event)) {
            $updated = $syncService->markItemDeleted($bitrixId, 'contacts');
            $this->logContactDeleteActivity($bitrixId);
        ...
        if ($this->isContactEvent($event)) {
            $synced = $syncService->syncSingleItemByBitrixId($bitrixId, 'contacts');
            if (! $synced) {
        ...
        if ($this->isDynamicItemEvent($event)) {
            ...
            if ($event === 'ONCRMITEMDELETE') {
                $updated = $syncService->markItemDeleted($bitrixId, $entity);
                $this->logDynamicDeleteActivity($entity, $bitrixId);
            ...
            $synced = $syncService->syncSingleItemByBitrixId($bitrixId, $entity);
            if (! $synced) {
    ...
    private function logContactDeleteActivity(int $bitrixId): void
    {
    ...
    private function logDynamicDeleteActivity(string $entity, int $bitrixId): void
    {
