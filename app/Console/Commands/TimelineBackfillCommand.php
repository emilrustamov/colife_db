...
class TimelineBackfillCommand extends Command
{
    ...
    protected $description = '[OPS one-off] Backfill old_values for recent bitrix.contact.updated events. Not scheduled.';
    ...
    public function handle(): int
    {
        ...
        DB::transaction(function () use ($candidates, $dryRun, &$updated, &$skipped): void {
            foreach ($candidates as $event) {
                $previousValues = $this->prevValues($event);
     ...
     */
    private function prevValues(object $event): ?array
    {
